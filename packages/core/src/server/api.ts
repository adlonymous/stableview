...
fastify.get('/api/stablecoins/:id', async (request, reply) => {
  try {
    ...
    const rows = await db.select().from(stablecoins);
    const item = Array.isArray(rows) ? rows.find((r: any) => r.id === numericId) : null;
...
fastify.get('/api/dashboard/stats', async (request, reply) => {
  try {
    ...
    const allStablecoins = await db.select({
      name: stablecoins.name,
      totalSupply: stablecoins.totalSupply,
    }).from(stablecoins).orderBy(sql`CAST(${stablecoins.totalSupply} AS DECIMAL) DESC`);

    const dominantStablecoin = allStablecoins.length > 0 ? allStablecoins[0].name : 'N/A';
    const dominantStablecoinShare = allStablecoins.length > 0 && totalSupply > 0 
