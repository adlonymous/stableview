...
export const stablecoinRouter = router({
  ...
  create: publicProcedure
    .input(createStablecoinSchema)
    .mutation(async ({ ctx, input }) => {
      // Check if stablecoin with this slug already exists
      const existing = await ctx.db.select({ id: stablecoins.id })
        .from(stablecoins)
      ...
      const result = await ctx.db.insert(stablecoins).values({
        slug: input.slug,
        name: input.name,
        token: input.token,
        ...
        transactionCount30d: input.transactionCount30d?.toString(),
        totalSupply: input.totalSupply?.toString(),
        uniqueAddresses30d: input.uniqueAddresses30d?.toString(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      Object.keys(updateData).forEach(key => {
        if (updateData[key as keyof typeof updateData] !== undefined) {
          ...
          if (['transactionVolume30d', 'transactionCount30d', 'totalSupply', 'uniqueAddresses30d', 'price'].includes(key) && 
      ...
      const result = await ctx.db.update(stablecoins)
        .set({
          ...updateValues,
          updatedAt: new Date(),
        })
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(stablecoins)
        .where(eq(stablecoins.id, input));
