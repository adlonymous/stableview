...
export class TopLedgerClient {
  ...
  async getLatestDataForMint(mint: string): Promise<{
    ...
    try {
      ...
      const recentVolumeData = volumeData
        .filter(item => 
          item.mint === mint && 
          new Date(item.block_date) >= thirtyDaysAgo
        );

      // Calculate 30-day volume and transaction count
      const volume30d = recentVolumeData.reduce((sum, item) => sum + (item.volume || 0), 0);
      const transactionCount30d = recentVolumeData.reduce((sum, item) => sum + (item.fee_payer || 0), 0);
  ...
  async getAllAvailableStablecoins(): Promise<Array<{
    slug: string;
    name: string;
    mint: string;
    totalSupply: number;
