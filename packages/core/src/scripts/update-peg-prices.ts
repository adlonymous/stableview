...
export async function updatePegPrices() {
  ...
  try {
    ...
    console.log(`Scheduled peg price update completed: ${successCount} successful, ${errorCount} failed.`);
    
    if (successCount > 0) {
      console.log('Successfully updated peg prices for:');
      results.filter(r => r.success).forEach(result => {
        console.log(`  - Stablecoin ${result.stablecoinId} (${result.peggedAsset}): $${result.pegPrice}`);
      });
    }
    ...
    if (errorCount > 0) {
      ...
      results.filter(r => !r.success).forEach(result => {
        console.error(`  - Stablecoin ${result.stablecoinId} (${result.peggedAsset}): ${result.error}`);
      });
    }
